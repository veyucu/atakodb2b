                            ...
                            <table class="table">
                                ...
                                <tfoot>
                                    ...
                                    <tr class="table-primary">
                                        ...
                                        <td class="text-end"><strong>{{ number_format($order->total, 2, ',', '.') }}
                                                ₺</strong></td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    <option value="pending" {{ $order->status == 'pending' ? 'selected' : '' }}>Beklemede
                                    </option>
                                    <option value="confirmed" {{ $order->status == 'confirmed' ? 'selected' : '' }}>Onaylandı
                                    </option>
                                    <option value="processing" {{ $order->status == 'processing' ? 'selected' : '' }}>
                                        Hazırlanıyor</option>
                                    <option value="shipped" {{ $order->status == 'shipped' ? 'selected' : '' }}>Kargoda
                                    </option>
                                    <option value="delivered" {{ $order->status == 'delivered' ? 'selected' : '' }}>Teslim
                                        Edildi</option>
                                    <option value="cancelled" {{ $order->status == 'cancelled' ? 'selected' : '' }}>İptal
                                    </option>
                                </select>
                            </div>
