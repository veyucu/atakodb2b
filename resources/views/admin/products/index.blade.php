                ...
                <div class="row g-2 align-items-center">
                    <div class="col-md-2">
                        <select name="search_type" class="form-select form-select-sm">
                            ...
                            <option value="barkod" {{ request('search_type') == 'barkod' ? 'selected' : '' }}>Barkod</option>
                            <option value="urun_kodu" {{ request('search_type') == 'urun_kodu' ? 'selected' : '' }}>Ürün Kodu</option>
                            <option value="urun_adi" {{ request('search_type') == 'urun_adi' ? 'selected' : '' }}>Ürün Adı</option>
                            <option value="muadil_kodu" {{ request('search_type') == 'muadil_kodu' ? 'selected' : '' }}>Muadil Kodu</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-1">
                        <select name="marka" class="form-select form-select-sm">
                            ...
                            @foreach($markalar as $marka)
                                <option value="{{ $marka }}" {{ request('marka') == $marka ? 'selected' : '' }}>{{ $marka }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-1">
                        <select name="grup" class="form-select form-select-sm">
                            ...
                            @foreach($gruplar as $grup)
                                <option value="{{ $grup }}" {{ request('grup') == $grup ? 'selected' : '' }}>{{ $grup }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-1">
                    ...
                    <div class="col-md-2">
                        <select name="sort" class="form-select form-select-sm">
                            ...
                            <option value="created_at_asc" {{ request('sort') == 'created_at_asc' ? 'selected' : '' }}>Eski→Yeni</option>
                            <option value="updated_at_desc" {{ request('sort') == 'updated_at_desc' ? 'selected' : '' }}>Güncelleme↓</option>
                            <option value="urun_kodu_asc" {{ request('sort') == 'urun_kodu_asc' ? 'selected' : '' }}>Kod A→Z</option>
                            <option value="urun_kodu_desc" {{ request('sort') == 'urun_kodu_desc' ? 'selected' : '' }}>Kod Z→A</option>
                            <option value="urun_adi_asc" {{ request('sort') == 'urun_adi_asc' ? 'selected' : '' }}>İsim A→Z</option>
                            <option value="urun_adi_desc" {{ request('sort') == 'urun_adi_desc' ? 'selected' : '' }}>İsim Z→A</option>
                        </select>
                    </div>
                    <div class="col-md-1">
                        ...
                        @if(request()->hasAny(['search', 'marka', 'grup', 'is_active', 'ozel_liste']))
                            <a href="{{ route('admin.products.index') }}" class="btn btn-secondary btn-sm w-100 mt-1" style="padding: 0.15rem;">
                    ...
                    <table class="table table-hover table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="text-center align-middle" style="width: 120px;">
                                    <div>Mal Fazlası</div>
                                    <small class="text-muted">Net Fiyat</small>
                                </th>
                            ...
                            @foreach($products as $product)
                                <tr id="product-row-{{ $product->id }}" data-product-id="{{ $product->id }}">
                                    ...
                                    <td class="text-center align-middle">
                                        <span class="view-mode">
                                            @if($product->is_active)
                                                <i class="fas fa-toggle-on text-primary" title="Aktif" style="font-size: 1.3rem;"></i>
                                            @else
                                                <i class="fas fa-toggle-off text-muted" title="Pasif" style="font-size: 1.3rem;"></i>
                                            @endif
                                        </span>
                                        <span class="edit-mode d-none">
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @else
                                            <div class="bg-light text-center d-inline-block" style="width: 50px; height: 50px; line-height: 50px;">
                                    ...
                                    <td class="align-middle">
                                        ...
                                        <span class="edit-mode d-none">
                                            <div><strong class="text-primary">{{ $product->urun_kodu }}</strong></div>
                                            <input type="text" class="form-control form-control-sm mt-1" name="urun_adi" value="{{ $product->urun_adi }}">
                                        </span>
                                    </td>
                                    <td class="align-middle">
                                        <span class="view-mode">
                                        ...
                                        <span class="edit-mode d-none">
                                            <input type="text" class="form-control form-control-sm mb-1" name="marka" value="{{ $product->marka }}" placeholder="Marka">
                                            <input type="text" class="form-control form-control-sm" name="grup" value="{{ $product->grup }}" placeholder="Grup">
                                        </span>
                                    </td>
                                    <td class="align-middle">
                                        <span class="view-mode">
                                        ...
                                        <span class="edit-mode d-none">
                                            <input type="text" class="form-control form-control-sm" name="muadil_kodu" value="{{ $product->muadil_kodu }}">
                                        </span>
                                    </td>
                                    <td class="text-end align-middle">
                                        <span class="view-mode">
                                            <strong class="text-primary">{{ number_format($product->satis_fiyati, 2, ',', '.') }} ₺</strong>
                                        </span>
                                        <span class="edit-mode d-none">
                                            <div class="input-with-suffix">
                                                <input type="number" class="form-control form-control-sm text-end" name="satis_fiyati" value="{{ $product->satis_fiyati }}" step="0.01">
                                    ...
                                    <td class="text-center align-middle">
                                        <span class="view-mode">
                                            <span class="badge bg-purple text-white" style="background-color: #6f42c1;">%{{ number_format($product->eczaci_kari ?? 0, 2) }}</span>
                                        </span>
                                        <span class="edit-mode d-none">
                                            <div class="input-with-suffix">
                                                <input type="number" class="form-control form-control-sm text-end" name="eczaci_kari" value="{{ $product->eczaci_kari }}" step="0.01">
                                                <span class="suffix">%</span>
                                            </div>
                                        </span>
                                    </td>
                                    <td class="text-center align-middle">
                                        <span class="view-mode">
                                            <span class="badge bg-warning text-dark">%{{ number_format($product->kurum_iskonto ?? 0, 2) }}</span>
                                        </span>
                                        <span class="edit-mode d-none">
                                            <div class="input-with-suffix">
                                                <input type="number" class="form-control form-control-sm text-end" name="kurum_iskonto" value="{{ $product->kurum_iskonto }}" step="0.01">
                                                <span class="suffix">%</span>
                                            </div>
                                        </span>
                                    </td>
                                    <td class="text-center align-middle">
                                        <span class="view-mode">
                                            <span class="badge bg-secondary">%{{ number_format($product->ticari_iskonto ?? 0, 2) }}</span>
                                        </span>
                                        <span class="edit-mode d-none">
                                            <div class="input-with-suffix">
                                                <input type="number" class="form-control form-control-sm text-end" name="ticari_iskonto" value="{{ $product->ticari_iskonto }}" step="0.01">
                                                <span class="suffix">%</span>
                                            </div>
                                        </span>
                                    </td>
                                    <td class="text-end align-middle">
                                        ...
                                        <span class="edit-mode d-none">
                                            <div class="input-with-suffix">
                                                <input type="number" class="form-control form-control-sm text-end" name="depocu_fiyati" value="{{ $product->depocu_fiyati }}" step="0.01">
                                                <span class="suffix">₺</span>
                                            </div>
                                        </span>
                                    </td>
                                    <td class="text-center align-middle">
                                        <span class="view-mode">
                                            <div class="mb-1">
                                                @if($product->mf)
                                                    <span class="badge bg-success">{{ $product->mf }}</span>
                                                @else
                                                    <span class="text-muted">-</span>
                                                @endif
                                            </div>
                                            <div>
                                                <strong class="text-success">{{ $product->net_fiyat_manuel ? number_format($product->net_fiyat_manuel, 2, ',', '.') . ' ₺' : '-' }}</strong>
                                            </div>
                                        </span>
                                        <span class="edit-mode d-none">
                                            <input type="text" class="form-control form-control-sm mb-1 text-center" name="mf" value="{{ $product->mf }}" placeholder="MF">
                                            <div class="input-with-suffix">
                                                <input type="number" class="form-control form-control-sm text-end" name="net_fiyat_manuel" value="{{ $product->net_fiyat_manuel }}" step="0.01" placeholder="Net F.">
                                                <span class="suffix">₺</span>
                                            </div>
                                        </span>
                                    </td>
                                    <td class="text-center align-middle">
                                        ...
                                        <span class="edit-mode d-none">
                                            <div class="input-with-suffix mb-1">
                                                <input type="number" class="form-control form-control-sm text-end" name="kdv_orani" value="{{ $product->kdv_orani }}" step="1">
                                                <span class="suffix">%</span>
                                            </div>
                                            <input type="number" class="form-control form-control-sm text-end" name="bakiye" value="{{ $product->bakiye }}" step="1">
                                        </span>
                                    </td>
                                    <td class="text-center align-middle">
                                        <span class="view-mode">
                                            <button type="button" class="btn btn-sm btn-info btn-quick-edit mb-1" title="Hızlı Değiştir">
                                        ...
                                        <span class="edit-mode d-none">
                                            <button type="button" class="btn btn-sm btn-success btn-save-quick mb-1" title="Kaydet" data-product-id="{{ $product->id }}">
                                                <i class="fas fa-check"></i>
                                            </button>
                                            <button type="button" class="btn btn-sm btn-secondary btn-cancel-quick" title="İptal">
                                                <i class="fas fa-times"></i>
                                            </button>
                                        </span>
                                    </td>
...
function hesaplaDepocuFiyatiRow(row) {
    ...
    // Net fiyatı da hesapla
    hesaplaNetFiyatRow(row);
}

// Net fiyatı hesapla (her satır için)
function hesaplaNetFiyatRow(row) {
    const depocuFiyati = parseFloat(row.find('input[name="depocu_fiyati"]').val()) || 0;
    const mf = row.find('input[name="mf"]').val().trim();
    
    let netFiyat = depocuFiyati;
    
    // MF'yi parse et (örn: 10+5)
    if (mf && mf.includes('+')) {
    ...
    // Net fiyata yaz
    row.find('input[name="net_fiyat_manuel"]').val(netFiyat.toFixed(2));
}
...
$(document).ready(function() {
    ...
    $(document).on('input', 'input[name="depocu_fiyati"], input[name="mf"]', function() {
        const row = $(this).closest('tr');
        hesaplaNetFiyatRow(row);
    });
    ...
    $(document).on('click', '.btn-save-quick', function() {
        ...
        const data = {
            ...
            mf: row.find('input[name="mf"]').val(),
            net_fiyat_manuel: row.find('input[name="net_fiyat_manuel"]').val(),
