                            ...
                            <table class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center hide-on-mobile desktop-only-cell" style="width: 100px;">Mal Fazlası</th>
                                        <th class="text-end hide-on-mobile desktop-only-cell" style="width: 120px;">Net Fiyat</th>
                                        <th class="text-center mobile-combined-price-cell" style="display: none;">
                                            <span class="show-on-mobile">MF / Fiyat</span>
                                        </th>
                                        <th class="text-center hide-on-mobile desktop-only-cell" style="width: 150px;">Miktar</th>
                                        <th class="text-end hide-on-mobile desktop-only-cell" style="width: 120px;">Toplam</th>
                                ...
                                <tbody>
                                    @foreach($cartItems as $item)
                                        @php
                                            $netFiyat = $item->product->net_fiyat_manuel ?? $item->product->net_price ?? $item->price;
                                            $toplam = $netFiyat * $item->quantity;
                                        @endphp
                                        <tr id="cart-row-{{ $item->id }}">
                                            <!-- Image - Hidden on mobile -->
                                            <td class="hide-on-mobile">
                                                <a href="javascript:void(0)" onclick="showProductModal({{ $item->product->id }})" style="cursor: pointer;">
                                                    ...
                                                    @else
                                                        <div class="img-thumbnail d-flex align-items-center justify-content-center" 
                                                            ...
                                                            <i class="fas fa-camera text-secondary" style="font-size: 1.5rem; opacity: 0.4;"></i>
                                                        </div>
                                                    @endif
                                                </a>
                                            </td>
                                            ...
                                            <td class="product-name-cell">
                                                <a href="javascript:void(0)" onclick="showProductModal({{ $item->product->id }})" class="text-decoration-none">
                                                    <strong class="text-dark">{{ $item->product->urun_adi }}</strong>
                                                </a>
                                                <br class="hide-on-mobile">
                                                <small class="text-muted hide-on-mobile">Kod: {{ $item->product->urun_kodu }}</small>
                                            </td>
                                            ...
                                            <td class="text-center mobile-combined-price-cell" style="display: none;">
                                                <div class="mf-section">
                                                    @if($item->mal_fazlasi > 0)
                                                        <span class="badge bg-success" style="font-size: 0.65rem; padding: 0.15rem 0.3rem;">
                                                            {{ $item->quantity }}+{{ $item->mal_fazlasi }}
                                                        </span>
                                                    @elseif($item->product->mf)
                                                        <span style="font-size: 0.7rem;">{{ $item->product->mf }}</span>
                                                    @else
                                            ...
                                            <td class="text-center mobile-qty-delete-cell" style="display: none;">
                                                <div class="input-group input-group-sm" id="cart-qty-group-mobile-{{ $item->id }}">
                                                    <button class="btn btn-outline-secondary" 
                                                            type="button"
                                                            onclick="decreaseCartQuantity({{ $item->id }})"
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mb-3">
                            ...
                            <strong class="h5 text-primary" id="grand-total">{{ number_format($totalWithVat, 2, ',', '.') }} ₺</strong>
                        </div>
    ...
    function showFlyingNotification(buttonElement, message) {
        ...
        if (cartRect) {
            setTimeout(function() {
                ...
                notification.style.top = (cartRect.top) + 'px';
                notification.style.transform = 'scale(0.5)';
                notification.style.opacity = '0';
            }, 600);
        } else {
            setTimeout(function() { notification.style.opacity = '0'; }, 1000);
        }
        
        setTimeout(function() { notification.remove(); }, 1200);
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotification(response.message || 'Miktar güncellenirken hata oluştu', 'error');
                }
            },
            error: function(xhr) {
                console.error('Update error:', xhr);
                let message = 'Miktar güncellenirken hata oluştu';
                let oldQuantity = $('#cart-qty-' + cartId).data('old-value') || 1;
                let showInlineWarning = false;
                
                // Detailed error handling
                if (xhr.status === 400) {
                    // Stok yetersiz veya validation hatası
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        message = xhr.responseJSON.message;
                        showInlineWarning = true; // Stok hatası için inline göster
                    }
                    // Eski değere geri dön (both desktop and mobile)
                    $('#cart-qty-' + cartId).val(oldQuantity);
                    $('#cart-qty-mobile-' + cartId).val(oldQuantity);
                } else if (xhr.status === 419) {
                    message = 'CSRF Token hatası. Sayfayı yenileyin.';
                ...
                // Stok hatası için inline uyarı göster
                if (showInlineWarning) {
                    showCartQuantityWarning(cartId, message);
                } else {
                    showNotification(message, 'error');
                }
            }
